...
public class ChatClientServerThread extends Thread
{
		...
		public ChatClientServerThread(Socket sock, List<ObjectOutputStream> listWriters)
		{
				this.sock = sock;
		...
		public void run() 
		{
				try
				{
						   while(true)
					      {
							   		...
							   		try 
							   		{
								   			uData = new File("UserData.txt");
									   		fData = new File("FriendData.txt"); 
									   		tData = new File("TempData.txt");
									   		if(!uData.exists())uData.createNewFile();
									   		if(!fData.exists())fData.createNewFile();
									   		if(!lData.exists())lData.createNewFile();
									   		tData.createNewFile();
							   	  ...
							   	  else if(message.get(0).equals("add_chatroom"))
							   	  {
									   		...
									   		if(!cData.exists())cData.createNewFile();
								  ...
								  else if(message.get(0).equals("rcv_chatrooms"))
							   	  {
									   		...
									   		{
									   				chatroomList.set(0,"send_chatrooms");
